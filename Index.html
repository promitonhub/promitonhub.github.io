<!doctype html>
<!--
Geometric Resonance — by Promit Mitra
License: MIT (see LICENSE)

Attribution request (not required by the MIT License):
If used commercially, please credit “Geometric Resonance by Promit Mitra”.

Demo audio:
demo.mp3 © 2025 Promit Mitra. All rights reserved.
...
<head>
  ...
  <script type="text/javascript" src="https://unpkg.com/butterchurn@2.6.0/dist/butterchurn.min.js"></script>
  <script type="text/javascript"
    src="https://unpkg.com/butterchurn-presets@2.4.7/lib/butterchurnPresets.min.js"></script>
  <style>
    /* 
     * HELLO LEARNER! 👋
     * 
     * Welcome to the code behind the Geometric Resonance Visualizer.
     * This section is called CSS (Cascading Style Sheets). It controls how the app LOOKS.
     * Think of it like the paint and decoration on a house.
     * 
     * We use it here to:
     * 1. Make the background dark (#020204) for better contrast.
     * 2. Style the buttons and sliders to look modern and "glassy" (using backdrop-filter).
     * 3. Position the 3D canvas to cover the whole screen.
     */
  ...
  <script type="importmap">
    { "imports": {
      ...
      "milkdrop-preset-converter-common": "https://esm.sh/milkdrop-preset-converter-common"
    } }
  </script>
  <script src="https://unpkg.com/butterchurn@2.6.0/lib/butterchurn.min.js"></script>
...
<body>
  ...
  <div id="preset-modal">
    <h3 style="color:rgba(255,255,255,0.85);font-weight:300;margin-bottom:18px;letter-spacing:0.15em;font-size:14px;">
      PRESETS</h3>
  ...
  <!--
    ...
    This container holds all the buttons, sliders, and menus you see at the bottom.
    It's overlaid on top of the canvas.
  -->
  <div id="ui">
    <!--
      CONTROL BAR
      The row of buttons for Play, Mic, Record, etc.
    -->
    <div class="controls">
    ...
    <div class="settings-container">
      ...
      <div class="settings-panel active" data-panel="main">
        ...
        <div class="setting"><span class="setting-label">Complexity</span><input type="range" id="density" min="1"
            max="5" step="1" value="3" /></div>
        ...
        <div class="setting"><span class="setting-label">Smoothness</span><input type="range" id="smoothness" min="0"
            max="1" step="0.05" value="0.7" /></div>
        <div class="setting"><span class="setting-label">Volume</span><input type="range" id="volume" min="0" max="1"
        ...
        <div class="setting">
          ...
          <select id="visualMode">
            ...
            <option value="inverseMirrorV">Inverse Mirror V</option>
            <option value="inverseMirrorH">Inverse Mirror H</option>
            <option value="centerEdgeSwap">Center-Edge Swap</option>
          </select>
        </div>
        ...
        <div class="setting"><span class="setting-label">Spin Speed</span><input type="range" id="modelSpinSpeed"
            min="0" max="2" step="0.01" value="1" /></div>
        <div class="setting">
      ...
      <div class="settings-panel" data-panel="color">
        ...
        <div class="setting"><span class="setting-label">Primary</span><input type="color" id="colorPrimary"
            value="#ffffff" style="width:40px;height:30px;border:none;background:none;cursor:pointer;" /></div>
        <div class="setting"><span class="setting-label">Secondary</span><input type="color" id="colorSecondary"
            value="#4488ff" style="width:40px;height:30px;border:none;background:none;cursor:pointer;" /></div>
        <div class="setting"><span class="setting-label">Background</span><input type="color" id="colorBg"
            value="#020204" style="width:40px;height:30px;border:none;background:none;cursor:pointer;" /></div>
        ...
        <div class="setting"><span class="setting-label">Hue Rotate</span><input type="range" id="hueRotateSpeed"
            min="0" max="1" step="0.01" value="0" /></div>
      ...
      <div class="settings-panel" data-panel="camera">
        ...
        <div class="setting"><span class="setting-label">Distance</span><input type="range" id="cameraDistance" min="15"
            max="80" step="1" value="35" /></div>
        <div class="setting"><span class="setting-label">Speed</span><input type="range" id="cameraSpeed" min="0.1"
            max="2" step="0.05" value="0.6" /></div>
        <div class="setting"><span class="setting-label">Shake</span><input type="range" id="cameraShake" min="0"
            max="2" step="0.05" value="0.45" /></div>
        <div class="setting"><span class="setting-label">FOV</span><input type="range" id="cameraFov" min="30" max="120"
            step="5" value="60" /></div>
      ...
      <div class="settings-panel" data-panel="physics">
        ...
        <div class="setting"><span class="setting-label">Turbulence</span><input type="range" id="turbulence" min="0"
            max="2" step="0.05" value="0.25" /></div>
        <div class="setting"><span class="setting-label">Cohesion</span><input type="range" id="cohesion" min="0"
            max="1" step="0.05" value="0.55" /></div>
        <div class="setting">
      ...
      <div class="settings-panel" data-panel="layers">
        ...
        <div class="setting"><span class="setting-label">Particle Size</span><input type="range" id="particleSizeMult"
            min="0.5" max="2.5" step="0.05" value="1" /></div>
      ...
      <div class="settings-panel" data-panel="fx">
        <div class="setting"><span class="setting-label">Bloom</span><input type="range" id="bloomStrength" min="0"
            max="3" step="0.05" value="0.8" /></div>
        <div class="setting"><span class="setting-label">Glow Radius</span><input type="range" id="bloomRadius" min="0"
            max="2" step="0.05" value="0.7" /></div>
        <div class="setting">
        ...
        <div class="setting"><span class="setting-label">Glitch</span><input type="range" id="glitchAmount" min="0"
            max="1" step="0.05" value="0" /></div>
        <div class="setting"><span class="setting-label">Vignette</span><input type="range" id="vignette" min="0"
            max="1" step="0.05" value="0.35" /></div>
        <div class="setting"><span class="setting-label">Grain</span><input type="range" id="grain" min="0" max="1"
            step="0.05" value="0.12" /></div>
        <div class="setting"><span class="setting-label">RGB Split</span><input type="range" id="aberration" min="0"
            max="1" step="0.05" value="0.08" /></div>
        <div class="setting"><span class="setting-label">Anamorphic</span><input type="range" id="anamorphic" min="0"
            max="1" step="0.05" value="0" /></div>
        <div class="setting"><span class="setting-label">Scanlines</span><input type="range" id="scanlines" min="0"
            max="1" step="0.05" value="0" /></div>
        <div class="setting">
      ...
      <div class="settings-panel" data-panel="env">
        ...
        <div class="setting"><span class="setting-label">Rings</span><input type="range" id="ringCount" min="0" max="8"
            step="1" value="3" /></div>
        <div class="setting">
      ...
      <div class="settings-panel" data-panel="video">
        <div class="group-title"
          style="width:100%;text-align:center;margin-bottom:10px;color:rgba(255,255,255,0.5);font-size:10px;letter-spacing:0.2em;">
          LIVE VIDEO LAYER</div>

        <div class="setting"><span class="setting-label">Enable Video</span>
          <div class="toggle" id="toggleVideoLayer"></div>
        </div>
        ...
        <div class="setting"><span class="setting-label">Opacity</span><input type="range" id="videoOpacity" min="0"
        ...
        <div class="setting">
          ...
          <select id="videoMirror">
            <option value="none">None</option>
            <option value="x">Left-Right</option>
            <option value="y">Up-Down</option>
            <option value="xy">Quad (Kaleido)</option>
          </select>
        </div>

        <div class="setting"><span class="setting-label">Luma Key</span>
          <div class="toggle" id="toggleLumaKey"></div>
        </div>
        <div class="setting"><span class="setting-label">Key Thresh</span><input type="range" id="lumaThreshold" min="0"
            max="1" step="0.05" value="0.1" /></div>
        <div class="setting"><span class="setting-label">Key Smooth</span><input type="range" id="lumaSmooth" min="0"
            max="0.5" step="0.01" value="0.1" /></div>

        <div class="group-title"
          style="width:100%;text-align:center;margin:15px 0 10px 0;color:rgba(255,255,255,0.5);font-size:10px;letter-spacing:0.2em;">
          MILKDROP LAYER</div>

        <div class="setting"><span class="setting-label">Enable Milkdrop</span>
          <div class="toggle" id="toggleMilkdrop"></div>
        </div>
        <div class="setting"><span class="setting-label">Load Presets</span>
          <button class="btn" style="width:auto;padding:6px 12px;border-radius:6px;font-size:10px;"
            onclick="loadMilkdropFolder()">Folder</button>
          <button class="btn" style="width:auto;padding:6px 12px;border-radius:6px;font-size:10px;margin-left:4px;"
            onclick="loadBundledPresets()">Bundled</button>
        </div>

        <div class="setting"><span class="setting-label">MD Opacity</span><input type="range" id="milkdropOpacity"
            min="0" max="1" step="0.05" value="0.6" /></div>

        <div class="setting">
          <span class="setting-label">MD Blend</span>
          <select id="milkdropBlendMode">
        ...
        <div class="setting"><span class="setting-label">MD Luma Key</span>
          <div class="toggle" id="toggleMilkdropLumaKey"></div>
        </div>
        <div class="setting"><span class="setting-label">MD Key Thresh</span><input type="range"
            id="milkdropLumaThreshold" min="0" max="1" step="0.05" value="0.1" /></div>
        <div class="setting"><span class="setting-label">MD Key Smooth</span><input type="range" id="milkdropLumaSmooth"
            min="0" max="0.5" step="0.01" value="0.1" /></div>

        <div id="milkdrop-preset-list"
          style="width:100%;max-height:100px;overflow-y:auto;background:rgba(0,0,0,0.3);border-radius:4px;padding:4px;font-size:9px;color:#aaa;margin-top:8px;">
          No presets loaded...
        </div>
      </div>
    ...
    <div class="controls">
      ...
      <button class="btn" id="micBtn" title="Live Input">
        <svg viewBox="0 0 24 24">
      ...
      <div class="setting" id="audioInputContainer">
  ...
  <script type="module">
    /*
     * JAVASCRIPT LOGIC
     * This is the brain of the operation. It handles:
     * 1. Setting up the 3D scene (Three.js).
     * 2. Analyzing the audio frequencies.
     * 3. Updating the visuals 60 times per second.
     */

    // We import "Three.js", a powerful library for making 3D graphics in the browser.
    // Think of it like importing a game engine.
    import * as THREE from "three";
    // Removed static import to prevent app crash if module fails to load
    // import { convertPreset } from "milkdrop-preset-converter-common";

    // We also import "Post-Processing" effects.
    // These are like Instagram filters for our 3D scene (Bloom, Glitch, etc.).
    ...
    async function loadMilkdropPreset(presetNameOrHandle) {
      if (!visualizer) {
        initMilkdrop();
      }

      // Handle bundled preset by name (string)
      if (typeof presetNameOrHandle === 'string') {
        const bundledPresets = window.butterchurnPresets && window.butterchurnPresets.getPresets
          ? window.butterchurnPresets.getPresets()
          : {};
        const preset = bundledPresets[presetNameOrHandle];
        if (preset && visualizer) {
          visualizer.loadPreset(preset, 0);
          highlightPreset(presetNameOrHandle);
          return;
        } else {
          console.warn("Preset not found in bundled presets:", presetNameOrHandle);
          return;
        }
      }

      // Handle file handle (from folder picker)
      const handle = presetNameOrHandle;
      if (handle && handle.getFile) {
        try {
          const file = await handle.getFile();
          const text = await file.text();

          // Try to parse as JSON first (pre-converted presets)
          let preset;
          try {
            preset = JSON.parse(text);
            console.log("Loaded JSON preset:", handle.name);
          } catch (jsonErr) {
            // Not JSON - this is a raw .milk file
            // Unfortunately, raw .milk files require server-side conversion
            // because they contain HLSL shaders that need to be compiled to GLSL
            alert(
              "⚠️ Raw .milk files require conversion.\n\n" +
              "Butterchurn cannot directly load raw Milkdrop 3 presets (.milk files) in the browser.\n\n" +
              "Options:\n" +
              "1. Use the bundled presets (click 'Load Bundled Presets' button)\n" +
              "2. Use pre-converted JSON presets\n" +
              "3. Convert presets using the milkdrop-preset-converter tool offline"
            );
            return;
          }
          ...
          if (visualizer && preset) {
            visualizer.loadPreset(preset, 0);
            highlightPreset(handle.name.replace('.milk', '').replace('.json', ''));
          }
        } catch (err) {
    ...
    function highlightPreset(name) {
      const list = document.getElementById('milkdrop-preset-list');
      if (!list) return;
      Array.from(list.children).forEach(c => {
        if (c.textContent.startsWith('📁')) return;
        c.style.color = c.textContent === name ? '#fff' : '#aaa';
      });
    }
    ...
    /*
     * CONFIGURATION
     * This object holds all the settings for the visualizer.
     * When you change a slider or click a button, it updates these values.
     */
    const config = {
      ...
      milkdropOpacity: 0.6,
      milkdropBlendMode: 'add',
      milkdropLumaKey: false,
      milkdropLumaThreshold: 0.1,
      milkdropLumaSmooth: 0.1
    };
    ...
    /*
     * THE RENDERER
     * This is the engine that draws the 3D scene onto the <canvas> element.
     * We set it to fill the whole screen and use "tone mapping" for better colors.
     */
    ...
    /*
     * AUDIO ANALYZER
     * This class takes the raw sound data and makes sense of it.
     * It calculates:
     * 1. Volume (RMS)
     * 2. Bass, Mids, Highs (Frequency Bands)
     * 3. Beat Detection (Is there a kick drum?)
     */
    class AudioAnalyzer {
      constructor() {
        this.bands = 64;
    ...
    const screenFrag = /* glsl */`
      ...
      uniform float uVisualMode; // 0=standard,1=kaleidoscope,2=mirrorV,3=mirrorH,4=inverseMirrorV,5=inverseMirrorH,6=centerEdgeSwap
      ...
      void main() {
        ...
        // Visual mode effects
        if (uVisualMode > 0.5 && uVisualMode < 1.5) {
          ...
          uv = p + 0.5;
        } else if (uVisualMode > 1.5 && uVisualMode < 2.5) {
          // Mirror Vertical (top-bottom mirror)
          uv.y = uv.y < 0.5 ? uv.y : 1.0 - uv.y;
        } else if (uVisualMode > 2.5 && uVisualMode < 3.5) {
          // Mirror Horizontal (left-right mirror)
          uv.x = uv.x < 0.5 ? uv.x : 1.0 - uv.x;
        } else if (uVisualMode > 3.5 && uVisualMode < 4.5) {
          // Inverse Mirror Vertical (flip and mirror)
          uv.y = uv.y < 0.5 ? 1.0 - uv.y * 2.0 : (uv.y - 0.5) * 2.0;
        } else if (uVisualMode > 4.5 && uVisualMode < 5.5) {
          // Inverse Mirror Horizontal (flip and mirror)
          uv.x = uv.x < 0.5 ? 1.0 - uv.x * 2.0 : (uv.x - 0.5) * 2.0;
        } else if (uVisualMode > 5.5) {
          // Center-Edge Swap (asymmetrical)
          vec2 center = vec2(0.5, 0.5);
          vec2 toCenter = center - uv;
          float dist = length(toCenter);
          uv = center + toCenter * (1.0 - dist * 2.0);
          uv = fract(uv);
        }
    ...
    const screenFXPass = new ShaderPass({
      uniforms: {
        ...
        uKaleidoscope: { value: 0 }, uKaleidoscopeSegments: { value: 6 }, 
uVisualMode: { value: 0 },
uFilmLook: { value: 0 }, uBeatPulse: { value: 0 }, uGlitch: { value: 0 },
        uNegative: { value: 0 }
      },
    ...
    /*
     * AUDIO SETUP
     * This function prepares the "ears" of the application.
     * It connects the microphone or music file to an "Analyzer" that listens to the sound.
     */
    function initAudioContext() {
      ...
      // Store nodes for reconnection
      audioCtx.nodes = { highpass, compressor, analyser, gainNode, mediaDest };
    ...
    function initAudio() {
      initAudioContext();
      if (sourceNode) return; // Already initialized file source

      sourceNode = audioCtx.createMediaElementSource(audioEl);
      sourceNode.connect(audioCtx.nodes.highpass);
    }

    async function populateAudioInputs() {
      const select = document.getElementById('audioInputSelect');
      const container = document.getElementById('audioInputContainer');

      if (!navigator.mediaDevices || !navigator.mediaDevices.enumerateDevices) {
        alert("Audio input enumeration is not supported in this browser or context.");
        return;
      }
    ...
    async function startMicrophone(deviceId) {
      ...
      try {
        ...
        // Disconnect file source if it exists
        if (sourceNode) {
          try { sourceNode.disconnect(); } catch (e) { }
        }

        micSource = audioCtx.createMediaStreamSource(micStream);
        micSource.connect(audioCtx.nodes.highpass);

        // Mute output to speakers to prevent feedback/echo during live input
    ...
    function stopMicrophone() {
      ...
      // Reconnect output to speakers for file playback
    ...
    // --------------------------------------------------------------------
    // --------------------------------------------------------------------
    // Milkdrop Layer Implementation
    // --------------------------------------------------------------------
    let visualizer, milkdropCanvas, milkdropTexture, milkdropMesh;
    let milkdropPresets = {};
    let milkdropKeys = [];
    let milkdropAudioConnected = false; // Track audio connection state
    let milkdropLastSource = null; // Track which source is connected

    function initMilkdrop() {
      if (visualizer) return;

      // Ensure audio context is ready (crucial for butterchurn)
      initAudioContext();

      // Create canvas for butterchurn
      milkdropCanvas = document.createElement('canvas');
      milkdropCanvas.width = 800; // Lower res for performance
      milkdropCanvas.height = 600;

      // Initialize butterchurn
      try {
        // Handle both module and global script loading patterns
        const butterchurnLib = (window.butterchurn && window.butterchurn.default) ? window.butterchurn.default : window.butterchurn;

        if (!butterchurnLib) {
          throw new Error("Butterchurn library not loaded.");
        }

        visualizer = butterchurnLib.createVisualizer(audioCtx, milkdropCanvas, {
          width: 800,
          height: 600,
          pixelRatio: 1,
          textureRatio: 1,
        ...
        alert("Failed to initialize Milkdrop visualizer: " + err.message);
      }

      // Connect audio
      // We need to connect the sourceNode to the visualizer
      // But sourceNode might not be ready yet. We'll check in animate loop or when audio starts.
    }

    // Directory Navigation State
    let currentDirHandle = null;
    let dirStack = []; // Stack of handles for "Back" navigation

    window.loadMilkdropFolder = async () => {
      ...
      try {
        ...
        await renderFileList(currentDirHandle);

        // Auto-enable milkdrop if not already
        if (!config.milkdropEnabled) {
          config.milkdropEnabled = true;
          document.getElementById('toggleMilkdrop').classList.add('active');
        }
    ...
    // Load presets that are bundled with the butterchurn-presets library
    window.loadBundledPresets = () => {
      const list = document.getElementById('milkdrop-preset-list');
      list.innerHTML = 'Loading bundled presets...';

      const bundledPresets = window.butterchurnPresets && window.butterchurnPresets.getPresets
        ? window.butterchurnPresets.getPresets()
        : null;

      if (!bundledPresets || Object.keys(bundledPresets).length === 0) {
        list.innerHTML = 'No bundled presets available. Check if butterchurn-presets loaded.';
        return;
      }

      const presetNames = Object.keys(bundledPresets).sort();
      list.innerHTML = '';

      presetNames.forEach(name => {
        const item = document.createElement('div');
        item.textContent = name;
        item.style.cssText = 'cursor:pointer;padding:2px;color:#aaa;';
        item.onclick = () => loadMilkdropPreset(name);
      ...
      console.log("Loaded", presetNames.length, "bundled presets");
    };
      ...
      try {
        for await (const entry of dirHandle.values()) {
          if (entry.kind === 'directory') {
            folders.push(entry);
          } else if (entry.kind === 'file' && entry.name.endsWith('.milk')) {
            files.push(entry);
          }
        }
        ...
        if (dirStack.length > 0) {
          ...
          backItem.style.cssText = 'cursor:pointer;padding:4px;color:#ffcc00;font-weight:bold;border-bottom:1px solid rgba(255,255,255,0.1);';
          backItem.onclick = () => exitFolder();
        ...
        folders.forEach(folder => {
          ...
          item.style.cssText = 'cursor:pointer;padding:2px;color:#88ccff;';
          item.onclick = () => enterFolder(folder);
          list.appendChild(item);
        });
        ...
        files.forEach(file => {
          const item = document.createElement('div');
          item.textContent = file.name.replace('.milk', '');
          item.style.cssText = 'cursor:pointer;padding:2px;color:#aaa;';
          item.onclick = () => loadMilkdropPreset(file); // Pass handle directly
          list.appendChild(item);
        });

        if (folders.length === 0 && files.length === 0) {
          list.innerHTML = 'No .milk files or folders found.';
        }

      } catch (err) {
        console.error("Error reading directory:", err);
        list.innerHTML = 'Error reading directory.';
      }
    }

    async function enterFolder(handle) {
      dirStack.push(currentDirHandle);
    ...
    async function exitFolder() {
      ...
      currentDirHandle = dirStack.pop();
      await renderFileList(currentDirHandle);
    }



    function buildMilkdropMesh() {
      if (milkdropMesh) bgScene.remove(milkdropMesh);

      milkdropTexture = new THREE.CanvasTexture(milkdropCanvas);

      // Shader material with luma key support
      const mat = new THREE.ShaderMaterial({
        uniforms: {
          map: { value: milkdropTexture },
          opacity: { value: config.milkdropOpacity },
          lumaKeyEnabled: { value: 0.0 },
          lumaThreshold: { value: config.milkdropLumaThreshold },
          lumaSmooth: { value: config.milkdropLumaSmooth }
        },
        vertexShader: `
          varying vec2 vUv;
          void main() {
        ...
        fragmentShader: `
          ...
          uniform float lumaKeyEnabled;
          uniform float lumaThreshold;
          uniform float lumaSmooth;
          varying vec2 vUv;
          ...
          void main() {
            ...
            // Calculate alpha based on luma key
            float keyAlpha = 1.0;
            if (lumaKeyEnabled > 0.5) {
              keyAlpha = smoothstep(lumaThreshold, lumaThreshold + lumaSmooth, luma);
            }
            
            gl_FragColor = vec4(texColor.rgb, texColor.a * opacity * keyAlpha);
          }
        `,
        transparent: true,
        blending: THREE.AdditiveBlending,
      ...
      milkdropMesh = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), mat);
      bgScene.add(milkdropMesh);
    }

    function updateMilkdrop() {
      if (!config.milkdropEnabled || !visualizer) {
        if (milkdropMesh) milkdropMesh.visible = false;
        return;
      }

      // Ensure audio connection - handle both file (sourceNode) and mic (micSource)
      const activeSource = isLiveInput ? micSource : sourceNode;
      if (activeSource && (!milkdropAudioConnected || milkdropLastSource !== activeSource)) {
        try {
          visualizer.connectAudio(activeSource);
          milkdropAudioConnected = true;
          milkdropLastSource = activeSource;
          console.log("Milkdrop audio connected to", isLiveInput ? "microphone" : "file source");
        } catch (err) {
          console.error("Failed to connect audio to Milkdrop:", err);
        }
      }

      visualizer.render();

      if (!milkdropMesh) buildMilkdropMesh();

      milkdropMesh.visible = true;
      milkdropTexture.needsUpdate = true;

      // Update shader uniforms for luma key
      if (milkdropMesh.material.uniforms) {
        milkdropMesh.material.uniforms.opacity.value = config.milkdropOpacity;
        milkdropMesh.material.uniforms.lumaKeyEnabled.value = config.milkdropLumaKey ? 1.0 : 0.0;
        milkdropMesh.material.uniforms.lumaThreshold.value = config.milkdropLumaThreshold;
        milkdropMesh.material.uniforms.lumaSmooth.value = config.milkdropLumaSmooth;
      } else {
        milkdropMesh.material.opacity = config.milkdropOpacity;
      }

      // Blending - support all modes
      const blendModes = {
        'add': THREE.AdditiveBlending,
        'normal': THREE.NormalBlending,
        'screen': THREE.AdditiveBlending, // Screen approximation
        'multiply': THREE.MultiplyBlending,
        'difference': THREE.SubtractiveBlending,
        'exclusion': THREE.SubtractiveBlending,
        'overlay': THREE.NormalBlending // Overlay uses shader
      };
      milkdropMesh.material.blending = blendModes[config.milkdropBlendMode] || THREE.AdditiveBlending;
    }
    // --------------------------------------------------------------------
    let videoEl, videoTexture, videoStream;
    let videoMesh;
    ...
    async function startVideoLayer() {
      if (videoStream) {
      ...
      try {
        // Optimization: Lower resolution for background effect to save RAM/Texture memory
        const constraints = {
          video: {
            deviceId: config.videoSource === 'default' ? undefined : { exact: config.videoSource },
        ...
        document.getElementById('toggleVideoLayer').classList.remove('active');
      }
    }
    ...
    function stopVideoLayer() {
      ...
      if (videoEl) {
        videoEl.pause();
        videoEl.srcObject = null; // Release memory
      }
      if (videoMesh) {
        bgScene.remove(videoMesh);
        videoMesh.geometry.dispose();
        videoMesh.material.dispose();
        videoMesh = null;
      }
      // Optimization: Dispose texture to free GPU memory
      if (videoTexture) {
    ...
    function buildVideoMesh() {
      if (videoMesh) bgScene.remove(videoMesh);
      ...
      const frag = `
        ...
        uniform float uOpacity;
        uniform int uBlendMode; // 0=normal, 1=add, 2=screen, 3=overlay, 4=multiply, 5=diff
        uniform int uMirror;    // 0=none, 1=x, 2=y, 3=xy
        uniform bool uKey;
        ...
        vec3 blend(vec3 base, vec3 blend, int mode) {
          ...
          if (mode == 3) return base * (base + 2.0 * blend * (1.0 - base)); // approx overlay
          if (mode == 4) return base * blend;
        ...
        void main() {
          ...
          // Mirroring
          if (uMirror == 1) uv.x = abs(uv.x - 0.5) * 2.0;
          if (uMirror == 2) uv.y = abs(uv.y - 0.5) * 2.0;
          if (uMirror == 3) { uv.x = abs(uv.x - 0.5) * 2.0; uv.y = abs(uv.y - 0.5) * 2.0; }
          
          // 5-tap Blur to hide pixelation from low-res input
          vec2 off = vec2(0.002, 0.002); // Slightly more than 1px for artistic blur
          vec4 tex = texture2D(tVideo, uv);
          tex += texture2D(tVideo, uv + vec2(off.x, 0.0));
          tex += texture2D(tVideo, uv - vec2(off.x, 0.0));
          tex += texture2D(tVideo, uv + vec2(0.0, off.y));
          tex += texture2D(tVideo, uv - vec2(0.0, off.y));
          tex /= 5.0;
          ...
          if (uKey) {
            ...
            float mask = smoothstep(uKeyThresh, uKeyThresh + uKeySmooth, luma);
            alpha *= mask;
          }
      ...
      const mat = new THREE.ShaderMaterial({
        uniforms: {
          ...
          uOpacity: { value: config.videoOpacity },
          uBlendMode: { value: 0 },
          uMirror: { value: 0 },
          uKey: { value: config.lumaKey },
          uKeyThresh: { value: config.lumaThreshold },
          uKeySmooth: { value: config.lumaSmooth }
        },
        vertexShader: vert,
        fragmentShader: frag,
        transparent: true,
        depthWrite: false,
        blending: THREE.NormalBlending // We handle blending manually or via alpha
      });

      // Fullscreen quad in bgScene
      videoMesh = new THREE.Mesh(new THREE.PlaneGeometry(2, 2), mat);
      bgScene.add(videoMesh);
    }

    function updateVideoUniforms() {
      if (!videoMesh) return;
      const u = videoMesh.material.uniforms;
      u.uOpacity.value = config.videoOpacity;
      u.uKey.value = config.lumaKey;
      u.uKeyThresh.value = config.lumaThreshold;
      u.uKeySmooth.value = config.lumaSmooth;

      const modes = { normal: 0, add: 1, screen: 2, overlay: 3, multiply: 4, difference: 5 };
      u.uBlendMode.value = modes[config.videoBlendMode] || 0;

      const mirrors = { none: 0, x: 1, y: 2, xy: 3 };
      u.uMirror.value = mirrors[config.videoMirror] || 0;

      // Update blending based on mode for proper compositing with other BG elements
      if (config.videoBlendMode === 'add') videoMesh.material.blending = THREE.AdditiveBlending;
      else videoMesh.material.blending = THREE.NormalBlending;
    }
     ...
     * 1. Analyze the audio for the current split-second.
     * 2. Update the position of all the 3D objects.
     * 3. Draw the scene to the screen.
     * 4. Repeat!
     */
    function animate(timestamp) {
      ...
      let isBeat = false;
      if (analyser && (playing || isLiveInput)) {
      ...
      if (config.videoEnabled) updateVideoUniforms();
      if (config.milkdropEnabled) updateMilkdrop();
...
// Map visual mode strings to numeric values for shader
const visualModeMap = {
  'standard': 0,
  'kaleidoscope': 1,
  'mirrorV': 2,
...
screenFXPass.uniforms.uVisualMode.value = visualModeMap[config.visualMode] || 0;
...
screenFXPass.uniforms.uNegative.value = config.negativeSpace ? 1.0 : 0.0;
    ...
    document.getElementById('toggleVideoLayer').addEventListener('click', e => {
      config.videoEnabled = e.target.classList.contains('active');
      if (config.videoEnabled) startVideoLayer(); else stopVideoLayer();
    });
    document.getElementById('videoSource').addEventListener('change', e => { config.videoSource = e.target.value; if (config.videoEnabled) startVideoLayer(); });
    document.getElementById('videoOpacity').addEventListener('input', e => { config.videoOpacity = parseFloat(e.target.value); });
    document.getElementById('videoBlendMode').addEventListener('change', e => { config.videoBlendMode = e.target.value; });
    document.getElementById('videoMirror').addEventListener('change', e => { config.videoMirror = e.target.value; });
    document.getElementById('toggleLumaKey').addEventListener('click', e => { config.lumaKey = e.target.classList.contains('active'); });
    document.getElementById('lumaThreshold').addEventListener('input', e => { config.lumaThreshold = parseFloat(e.target.value); });
    document.getElementById('lumaSmooth').addEventListener('input', e => { config.lumaSmooth = parseFloat(e.target.value); });

    // Milkdrop Controls
    document.getElementById('toggleMilkdrop').addEventListener('click', e => { config.milkdropEnabled = e.target.classList.contains('active'); });
    document.getElementById('milkdropOpacity').addEventListener('input', e => { config.milkdropOpacity = parseFloat(e.target.value); });
    document.getElementById('milkdropBlendMode').addEventListener('change', e => { config.milkdropBlendMode = e.target.value; });
    document.getElementById('toggleMilkdropLumaKey').addEventListener('click', e => { config.milkdropLumaKey = e.target.classList.contains('active'); });
    document.getElementById('milkdropLumaThreshold').addEventListener('input', e => { config.milkdropLumaThreshold = parseFloat(e.target.value); });
    document.getElementById('milkdropLumaSmooth').addEventListener('input', e => { config.milkdropLumaSmooth = parseFloat(e.target.value); });
    ...
    micBtn.addEventListener('click', () => {
      if (isLiveInput) {
        ...
        // If this is the first time, populate inputs
        if (audioInputSelect.options.length === 0) {
          populateAudioInputs().then(() => {
            startMicrophone(audioInputSelect.value);
          });
        } else {
          startMicrophone(audioInputSelect.value);
        }
